...
NUM_CLASSES = len(process.chord_name_map) + len(process.quality_map)
...
INITIAL_LEARNING_RATE = 10e-5     # Initial learning rate.
...
def inference(inputs):
    ...
    # If this model ran on a single GPU, we could simplify this function
    ...
    # norm1
    norm1 = tf.nn.lrn(
        pool1, 4, bias=1.0, alpha=0.001 / 9.0,
    ...
    with tf.variable_scope('local3') as scope:
        # Move everything into depth for a single matrix multiply.
    ...
    with tf.variable_scope('softmax_linear') as scope:
        ...
        softmax_linear = tf.add(
            tf.matmul(local4, weights), biases,
            name=scope.name
        )
...
def loss(logits, targets):
    ...
    Args:
        ...
        targets: Targets, a 1-D tensor of shape [batch_size].
    Returns:
